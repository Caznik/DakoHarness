...
An extensible harness for coding agents. Provides a two-tier memory system (long-term MongoDB + short-term SQLite) and session logging. This is the **OpenCode adapter** — session logging runs through the `dako-logger` OpenCode plugin (OpenCode has no settings.json hooks). The memory MCP servers are shared with the Claude Code target.
...
**Registry freshness:** On session start, before reading the user's first task, check whether `.opencode/skill-registry.md` is stale relative to `.opencode/commands/`. The registry is stale if any `.opencode/commands/*.md` file has an mtime newer than `.opencode/skill-registry.md`, OR if `.opencode/skill-registry.md` does not exist. If stale, invoke `/registry-refresh` and emit one line in the existing format: `Registry refreshed — N skills indexed.` If fresh, do nothing — no log line, no tool call. If `.opencode/commands/` does not exist (e.g. plugin-only installs where commands live elsewhere), skip the check silently.

**After compaction:** Call `find_patterns` with `query: "context-snapshot"` and `project: "DakoHarness"` once. If a result is found, read it to understand where work was interrupted. No delete needed — STM TTL handles expiry automatically. If you need more than the snapshot — to recover a specific earlier exchange — use `/recall-session <topic>` to semantically retrieve relevant past turns from the project's message history.
...
The same snapshot structure is used by `/checkpoint` for on-demand saves. If `remember_pattern` fails (STM MCP down), note the failure — do not silently continue without saving.
...
A list of available slash commands is in `.opencode/skill-registry.md`. Consult it when a user's request sounds like a workflow task that might match a command (e.g. "search memory", "end the session", "save this pattern"). Run `/registry-refresh` after adding or removing a command file.
...
- **Project name is always `"DakoHarness"`, agent is always `"opencode"`** when calling memory tools in this project.
- Session transcripts are captured automatically by the `dako-logger` plugin — you do not need to log individual messages manually.
