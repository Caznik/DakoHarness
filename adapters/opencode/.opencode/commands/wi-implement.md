...
Delegate to the `wi-implementer` subagent using OpenCode's `task` tool (you can also trigger it with an `@wi-implementer` mention). Provide:
- the target subagent: `wi-implementer`
...
The sub-agent is defined in `.opencode/agents/wi-implementer.md`. It carries the full protocol (architecture review, TDD coding, QA loop with AC Pre-Check, regression, Replan Request) as its system prompt.

For the underlying protocol, see `.opencode/agents/wi-implementer.md`.
