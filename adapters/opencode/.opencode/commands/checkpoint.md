...
## When to use
Run manually at any point in a long session to ensure the current state is recoverable. Also triggered automatically every 15 turns by the AGENTS.md protocol.
...
- `content`:
  ...
  ```
- `reasoning`: `"Manual context checkpoint via /checkpoint"`

### 5. Report result
- On success: "Context snapshot saved to short-term memory."
- On failure (MCP not responding): "STM MCP is not reachable — snapshot not saved. Run /doctor to diagnose."
