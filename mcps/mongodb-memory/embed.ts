 ...
 */
export function shouldEmbedMessage(role: string, content: string): boolean {
  if (role === "tool") return false;
  const trimmed = content.trim();
  if (trimmed.length === 0) return false;
  if (trimmed.length < MESSAGE_MIN_LEN) return false;
  return true;
}
...
export async function embedTexts(texts: string[]): Promise<Float32Array[]> {
  ...
  for (const t of texts) {
    ...
    const tensor = await pipe(t, { pooling: "mean", normalize: true }) as { data: Float32Array };
    out.push(new Float32Array(tensor.data));
  }
  return out;
}

export function floatsToBytes(v: Float32Array): Buffer {
...
export function bytesToFloats(b: Buffer): Float32Array {
 ...
 */
export function rrfMerge<T>(ftsRanked: T[], vectorRanked: T[], limit: number, k = 60): T[] {
  if (ftsRanked.length === 0 && vectorRanked.length === 0) return [];
  if (ftsRanked.length === 0) return vectorRanked.slice(0, limit);
  if (vectorRanked.length === 0) return ftsRanked.slice(0, limit);
...
async function loadPipeline(): Promise<(text: string, opts: { pooling: string; normalize: boolean }) => Promise<{ data: Float32Array }>> {
  if (!_pipelinePromise) {
    ...
    _pipelinePromise = (async () => {
      ...
      const transformers = (mod as { pipeline?: unknown; default?: { pipeline?: unknown } });
      const pipelineFn = (transformers.pipeline ?? transformers.default?.pipeline) as
  ...
  return _pipelinePromise as Promise<(text: string, opts: { pooling: string; normalize: boolean }) => Promise<{ data: Float32Array }>>;
}
 ...
 */
export function stubEmbed(text: string): Float32Array {
  ...
  // Unit-normalize
  let norm = 0;
  for (let i = 0; i < STUB_DIM; i++) norm += (v[i] as number) * (v[i] as number);
  norm = Math.sqrt(norm);
  if (norm > 0) {
    for (let i = 0; i < STUB_DIM; i++) v[i] = (v[i] as number) / norm;
  }
  return v;
}

/** Convenience for the MCP tool boundary — returns the JSON-shaped ToolResult content. */
export async function embedQueryResult(text: string): Promise<{ embedding: string; model: string }> {
  const [vec] = await embedTexts([text]);
  if (!vec) throw new Error("embedTexts returned an empty result");
  return {
