...
type CollectionName = "memories" | "workitems" | "sessions" | "messages";
const COLLECTIONS: CollectionName[] = ["memories", "workitems", "sessions", "messages"];
...
type InsertedTracker = Record<CollectionName, ObjectId[]>;
...
export async function main(argv: string[]): Promise<number> {
  ...
  if (!sqlitePath) missing.push("DAKO_SQLITE_PATH");
  ...
  const inserted: InsertedTracker = { memories: [], workitems: [], sessions: [], messages: [] };
  let envRewritten = false;
  let envOriginal: { bytes: Buffer; existed: boolean } | null = null;
  ...
  try {
    ...
    const plans: Record<CollectionName, CollectionPlan> = {
    ...
    const results: Record<CollectionName, CollectionResult> = {
      ...
      if (plan.toInsert.length > 0) {
        try {
          ...
          const e = insertErr as {
            result?: { insertedIds?: Record<string, ObjectId> };
            insertedIds?: Record<string, ObjectId>;
            insertedDocs?: Array<{ _id?: ObjectId }>;
          };
          const partialMap = e?.result?.insertedIds ?? e?.insertedIds;
          if (partialMap) {
            inserted[col].push(...(Object.values(partialMap) as ObjectId[]));
          } else if (Array.isArray(e?.insertedDocs)) {
            for (const d of e.insertedDocs) {
              if (d && d._id) inserted[col].push(d._id);
            }
          }
          throw insertErr;
        }
    ...
    if (envRewritten && envOriginal) {
      try { fs.writeFileSync(ENV_PATH, envOriginal.bytes); }
      catch (e) { process.stderr.write(`Warning: failed to revert .env: ${(e as Error).message}\n`); }
    }
    ...
    if (mongoClient) {
      try { await mongoClient.close(); } catch { /* best effort */ }
    }
  }
}
...
function buildMemoriesPlan(sqlite: Database.Database): CollectionPlan {
  const rows = sqlite.prepare(`SELECT * FROM memories`).all() as Array<Record<string, unknown>>;
  const docs = rows.map((r) => {
    // Drop integer id; parse tags JSON; ISO string -> Date
    let tags: string[] = [];
    try { tags = JSON.parse((r["tags"] as string | null) ?? "[]") as string[]; }
    catch { tags = []; }
...
function buildWorkitemsPlan(sqlite: Database.Database): CollectionPlan {
  const rows = sqlite.prepare(`SELECT * FROM workitems`).all() as Array<Record<string, unknown>>;
  const docs = rows.map((r) => {
    const doc: Record<string, unknown> = {
      wi_path:       r["wi_path"],
      project:       r["project"],
      documentation: r["documentation"],
      archived_at:   new Date(r["archived_at"] as string),
    };
    if (r["username"]   !== null && r["username"]   !== undefined) doc["username"]   = r["username"];
    if (r["git_commit"] !== null && r["git_commit"] !== undefined) doc["git_commit"] = r["git_commit"];
    return doc;
  });
  return { readCount: rows.length, toInsert: docs, skipCount: 0 };
}

function buildSessionsPlan(sqlite: Database.Database): CollectionPlan {
  const rows = sqlite.prepare(`SELECT * FROM sessions`).all() as Array<Record<string, unknown>>;
  const docs = rows.map((r) => ({
...
function buildMessagesPlan(sqlite: Database.Database): CollectionPlan {
  const rows = sqlite.prepare(`SELECT * FROM messages`).all() as Array<Record<string, unknown>>;
  const docs = rows.map((r) => ({
    session_id: r["session_id"],
    role:       r["role"],
    content:    r["content"],
...
async function dedupAgainstMongo(db: Db, plans: Record<CollectionName, CollectionPlan>): Promise<void> {
  // memories: natural key = (project, agent, type, title)
  {
    ...
    for (const doc of plans.memories.toInsert) {
      ...
      if (set.has(key)) plans.memories.skipCount++;
      else { kept.push(doc); set.add(key); /* also dedup within batch */ }
    }
  ...
  // workitems: natural key = wi_path
  {
    ...
    for (const doc of plans.workitems.toInsert) {
      ...
      if (set.has(key)) plans.workitems.skipCount++;
      else { kept.push(doc); set.add(key); }
    }
  ...
  // sessions: natural key = session_id
  {
    ...
    for (const doc of plans.sessions.toInsert) {
      ...
      if (set.has(key)) plans.sessions.skipCount++;
      else { kept.push(doc); set.add(key); }
    }
  ...
  // messages: natural key = (session_id, seq)
  {
    ...
    for (const doc of plans.messages.toInsert) {
      ...
      if (set.has(key)) plans.messages.skipCount++;
      else { kept.push(doc); set.add(key); }
    }
...
async function rollback(db: Db, inserted: InsertedTracker): Promise<void> {
  for (const col of COLLECTIONS) {
...
export function rewriteEnvBackend(envPath: string, newValue: string): void {
...
function emptyResult(plan: CollectionPlan): CollectionResult {
  return { read: plan.readCount, inserted: 0, skipped: plan.skipCount, durationMs: 0 };
}

function printSummary(results: Record<CollectionName, CollectionResult>, bakPath: string, totalMs: number): void {
  process.stdout.write("\nSummary\n");
