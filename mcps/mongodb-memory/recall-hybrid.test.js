...
function cleanupSqlite(s, tmpDir) {
    void s.close();
    try {
...
test("[sqlite] inline embed happy path — remember writes embedding + model", async () => {
    ...
    try {
        ...
        const row = db.prepare(`SELECT embedding, embedding_model FROM memories LIMIT 1`).get();
        db.close();
...
test("[sqlite] inline embed failure — remember still inserts with null fields", async () => {
    ...
    try {
        ...
        const before = (new Database(dbPath, { readonly: true })).prepare(`SELECT COUNT(*) as c FROM memories`).get();
        await storage.remember({
            project: "P", agent: "claude-code", type: "decision",
            title: "T2", content: "C2",
        });
        const db2 = new Database(dbPath, { readonly: true });
        const after = db2.prepare(`SELECT COUNT(*) as c FROM memories`).get();
        const row = db2.prepare(`SELECT embedding, embedding_model FROM memories WHERE title='T2'`).get();
        db2.close();
        ...
        if (stubSave !== undefined)
            process.env["DAKO_EMBED_STUB"] = stubSave;
...
test("[sqlite] mode=vector throws helpful error when no embeddings exist", async () => {
    ...
    try {
        await assert.rejects(() => storage.recall({ project: "P", query: "anything", mode: "vector" }), /Run 'npm run embed-backfill'/);
    }
    finally {
        cleanupSqlite(storage, tmpDir);
    }
...
if (!MONGO_OK) {
    ...
    test("[mongo] inline embed + vector recall with mismatched-model exclusion", async () => {
        ...
        try {
            ...
            await assert.rejects(() => storage.recall({ project: "EmptyProj", query: "x", mode: "vector" }), /Run 'npm run embed-backfill'/);
        }
        finally {
...
//# sourceMappingURL=recall-hybrid.test.js.map