...
test("[sqlite] logMessage inline-embeds long user messages", async () => {
    ...
    try {
        ...
        const row = db.prepare(`SELECT embedding, embedding_model FROM messages LIMIT 1`).get();
        db.close();
...
test("[sqlite] logMessage skips empty / short / tool messages (AC-3)", async () => {
    ...
    try {
        ...
        const db = new Database(dbPath, { readonly: true });
        const rows = db.prepare(`SELECT embedding, embedding_model FROM messages ORDER BY seq`).all();
        db.close();
...
test("[sqlite] logMessage embed failure: row inserted with null fields (AC-2 failure)", async () => {
    ...
    try {
        ...
        await storage.logMessage({ session_id: "s1", role: "user", content: LONG_USER });
        const db = new Database(dbPath, { readonly: true });
        const row = db.prepare(`SELECT embedding, embedding_model, content FROM messages LIMIT 1`).get();
        db.close();
...
test("[sqlite] recallSessionMessages — invalid since throws (AC-5 validation)", async () => {
    ...
    try {
        await storage.startSession({ project: "P", agent: "claude-code", session_id: "s1" });
        await storage.logMessage({ session_id: "s1", role: "user", content: LONG_USER });
        await assert.rejects(() => storage.recallSessionMessages({ project: "P", query: "redis", since: "not-a-date" }), /expected ISO-8601/);
    }
    finally {
        cleanupSqlite(storage, tmpDir);
    }
...
//# sourceMappingURL=recall-session-messages.test.js.map