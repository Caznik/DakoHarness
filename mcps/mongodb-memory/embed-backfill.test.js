...
function seedDb(sqlitePath, preEmbeddedCount) {
...
function setupTestEnv(preEmbeddedCount) {
...
function countEmbedded(sqlitePath) {
    const db = new Database(sqlitePath, { readonly: true });
    const total = db.prepare(`SELECT COUNT(*) as c FROM memories`).get().c;
    const withVec = db.prepare(`SELECT COUNT(*) as c FROM memories WHERE embedding IS NOT NULL AND embedding_model = ?`).get(getModelId()).c;
    db.close();
...
test("--force re-embeds every row regardless of model match", async () => {
    ...
    try {
        ...
        const row = db2.prepare(`SELECT embedding FROM memories WHERE id = 1`).get();
        db2.close();
 ...
 * (skip-rule).
 */
function seedMessagesDb(sqlitePath, preEmbeddedCount) {
    fs.mkdirSync(dirname(sqlitePath), { recursive: true });
    const db = new Database(sqlitePath);
    db.exec(`
  ...
  `);
    ...
    ins.run("s1", "user", longA, 0, now, preEmbeddedCount >= 1 ? floatsToBytes(stubEmbed(`user: ${longA}`)) : null, preEmbeddedCount >= 1 ? currentModel : null);
    ins.run("s1", "assistant", longB, 1, now, preEmbeddedCount >= 2 ? floatsToBytes(stubEmbed(`assistant: ${longB}`)) : null, preEmbeddedCount >= 2 ? currentModel : null);
...
function countMessagesEmbedded(sqlitePath) {
    const db = new Database(sqlitePath, { readonly: true });
    const total = db.prepare(`SELECT COUNT(*) as c FROM messages`).get().c;
    const withVec = db.prepare(`SELECT COUNT(*) as c FROM messages WHERE embedding IS NOT NULL AND embedding_model = ?`).get(getModelId()).c;
    db.close();
    return { withVec, total };
}
function setupMessagesTestEnv(preEmbeddedMessages) {
    const envSnapshot = snapshotEnv();
...
test("--collection messages --force: re-embeds all eligible; skip-rule rows still skipped (AC-13 j)", async () => {
    ...
    try {
        ...
        assert.equal(code, 0);
        const db2 = new Database(env.sqlitePath, { readonly: true });
        const row = db2.prepare(`SELECT embedding FROM messages WHERE id = 1`).get();
        db2.close();
        assert.notDeepEqual(Array.from(row.embedding), Array.from(sentinel), "force should overwrite the sentinel");
...
test("--collection all: runs memories then messages (AC-13 k)", async () => {
  ...
  `);
    ...
    for (let i = 0; i < 3; i++)
        mIns.run("P", "claude-code", "decision", `t-${i}`, `c-${i}`, "[]", "project", null, now, null, null);
...
//# sourceMappingURL=embed-backfill.test.js.map