...
test("getModelId returns default when DAKO_EMBEDDING_MODEL is unset", () => {
  ...
  try {
    ...
    if (prev !== undefined) process.env["DAKO_EMBEDDING_MODEL"] = prev;
  }
});
...
test("getModelId returns env value when set", () => {
  ...
  try {
    ...
    if (prev === undefined) delete process.env["DAKO_EMBEDDING_MODEL"];
    else process.env["DAKO_EMBEDDING_MODEL"] = prev;
  }
});
