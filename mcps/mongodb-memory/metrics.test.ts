...
function rec(partial: Partial<WorkitemMetricsRecord>): WorkitemMetricsRecord {
  return {
...
function buildFixtureTree(): string {
  ...
  writeFile(join(root, "WI-alpha", "20260601-one", "implementation.md"),
    "## QA Log\n| It | AC | Res | Act |\n|---|---|---|---|\n| 1 | AC-1 | pass | x |\n\n## Plan Deviations\n| S | O | A | R |\n|---|---|---|---|\n| 1 | a | b | c |\n");
...
test("[sqlite] saveWorkitemMetrics upsert keeps one row per (wi, sub_feature) (AC-8)", async () => {
  ...
  try {
    ...
    const count = (db.prepare(`SELECT COUNT(*) c FROM workitem_metrics`).get() as { c: number }).c;
    const row = db.prepare(`SELECT qa_iterations FROM workitem_metrics WHERE wi=? AND sub_feature=?`).get("WI-x", "20260101-a") as { qa_iterations: number };
    db.close();
