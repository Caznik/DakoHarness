...
export function parseFrontmatter(md: string): Record<string, string> {
  ...
  for (const line of m[1]!.split(/\r?\n/)) {
    ...
    if (idx <= 0) continue; // no key, or garbled line → skip
 ...
 */
export function tableDataRows(md: string, headingRegex: RegExp): string[] {
  ...
  for (const row of rows) {
    ...
    if (!seenSeparator) {
      ...
      if (/^\|[\s:\-|]+\|?$/.test(row)) { seenSeparator = true; continue; }
 ...
 */
export function sectionText(md: string, headingRegex: RegExp): string {
  const lines = md.split(/\r?\n/);
  let start = -1;
  for (let i = 0; i < lines.length; i++) {
    const h = /^##\s+(.*)$/.exec(lines[i]!);
    if (h && headingRegex.test(h[1]!)) { start = i + 1; break; }
  }
  if (start === -1) return "";
 ...
 */
export function parseAcMetrics(reviewMd: string, warnings: string[]): {
  ac_total: number; ac_satisfied: number | null; ac_pass_rate: number | null; verdict: string | null;
} {
  ...
  for (const row of rows) {
    ...
    if (cell === "yes" || cell === "y") ac_satisfied++;
  }
...
export function parseQaIterations(implMd: string): number {
 ...
 */
export function parseReplans(implMd: string, sotMd: string): { deviation_count: number; dispatch_count: number } {
 ...
 */
export function parseGaps(reviewMd: string): { gaps_open: boolean; accepted_gaps: string } {
 ...
 */
export function parsePhaseDays(phaseDates: PhaseDate[], createdDate: string | null): {
  phase_days: Record<string, number>; total_days: number;
} {
...
function wholeDays(fromTs: number, toTs: number): number {
...
const PHASE_FILES: Array<{ phase: string; file: string }> = [
 ...
 */
export function harvestTree(workitemRoot: string, project: string): {
  records: WorkitemMetricsRecord[]; warnings: string[];
} {
  const topWarnings: string[] = [];
  const records: WorkitemMetricsRecord[] = [];
  ...
  for (const wi of wiDirs) {
    if (!wi.startsWith("WI-")) continue; // AC-1: skip non-WI entries
    const wiPath = join(workitemRoot, wi);
    if (!isDir(wiPath)) continue;
    ...
    for (const sub of subDirs) {
      ...
      if (!/^\d{8}-/.test(sub)) continue; // only <YYYYMMDD>-<sub-feature> folders
...
function buildRecord(wi: string, sub: string, project: string, subPath: string, sotMd: string): WorkitemMetricsRecord {
  ...
  const phaseDates: PhaseDate[] = PHASE_FILES.map(({ phase, file }) => {
...
function subPrefixDate(sub: string): string | null {
  const m = /^(\d{4})(\d{2})(\d{2})-/.exec(sub);
  return m ? `${m[1]}-${m[2]}-${m[3]}` : null;
}

function emptyRecord(wi: string, sub: string, project: string, warnings: string[]): WorkitemMetricsRecord {
  return {
    wi, sub_feature: sub, project,
 ...
 */
export function computeRollup(records: WorkitemMetricsRecord[]): WorkitemMetricsRollup {
  ...
  const wi_count_by_status: Record<string, number> = {};
