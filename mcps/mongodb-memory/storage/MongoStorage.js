...
export class MongoStorage {
    ...
    async logMessage(args) {
        ...
        if (shouldEmbedMessage(role, content)) {
            try {
                ...
                if (vec) {
                    await messages.updateOne({ _id: result.insertedId }, { $set: { embedding: new Binary(floatsToBytes(vec), 0), embedding_model: getModelId() } });
                }
            }
            catch (err) {
                const reason = err instanceof Error ? err.message : String(err);
    ...
    async recallSessionMessages(args) {
        ...
        const formatted = top.map((m) => {
            ...
            return `[${sid.slice(0, 8)}] [${tsIso}] [${m["role"]}]: ${m["content"]}`;
        }).join("\n\n");
